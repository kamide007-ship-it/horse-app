...
class User(UserMixin, db.Model):
    ...
    def monthly_limit(self) -> int | None:
        """Return monthly evaluation limit for the plan. None = unlimited."""
        if self.plan == "free":
            return 1  # total limit enforced separately
    ...
    def can_eval(self) -> bool:
        ...
        if limit is None:
            return True
        return self.quota_used_month < limit
...
class Evaluation(db.Model):
    ...
    front_photo_path = db.Column(db.String(512), nullable=True)
    rear_photo_path = db.Column(db.String(512), nullable=True)
...
class PaymentRequest(db.Model):
    """Bank transfer payment request (Stripe removed).

    Flow:
      - user selects a paid plan
      - a pending request is created with a reference code
      - user transfers and includes the reference
      - admin approves -> user's plan is updated
    """
